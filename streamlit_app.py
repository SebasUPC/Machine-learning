...
def get_clinical_service(_health: pd.DataFrame) -> dict:
    ...
    if joblib is not None and rf_path is not None:
        try:
            ...
            models = {}
            load_error = (
                f"No se pudieron cargar los .joblib de clasificacion ({ex}). "
                "Use scikit-learn==1.6.1 (misma version que Colab): pip install -r requirements.txt"
            )
...
def get_stock_service(_stock: pd.DataFrame) -> dict:
    ...
    if joblib is not None and path_t7 is not None and path_t14 is not None:
        try:
            models["t+7"] = joblib.load(path_t7)
            models["t+14"] = joblib.load(path_t14)
            ...
            return {
                ...
                "targets": targets, "source": source, "load_error": None,
            }
        except Exception as ex:
            models = {}
            load_error = f"No se pudieron cargar los modelos de regresion: {ex}"
...
[Kaggle / OCR / Formulario] --> [db_infrastructure.py] --> [SQLite: aldimi.db]
    ...
    arch = pd.DataFrame(
        [
            ...
            ["Capa de servicio", "Carga de joblib (Colab) o entrenamiento en cache", "streamlit_app.py + models/*.joblib"],
...
def render_sidebar(health, stock, clin, stock_srv) -> str:
    ...
    with st.sidebar.expander("Ingestion en Tiempo Real (OCR)", expanded=False):
    ...
    st.sidebar.markdown("- Alertas de stock a 7 y 14 dias")
