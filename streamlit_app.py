...
def train_risk_models(health: pd.DataFrame) -> tuple[dict[str, object], pd.DataFrame, list[str], LabelEncoder, pd.DataFrame]:
    ...
    return fitted, metrics, features, label_encoder, confusion_df


@st.cache_resource(show_spinner=False)
def train_stock_models(stock: pd.DataFrame) -> tuple[dict[str, object], pd.DataFrame, list[str]]:
...
def render_model_metrics(risk_metrics: pd.DataFrame, stock_metrics: pd.DataFrame) -> None:
    ...
    if XGBClassifier is None or XGBRegressor is None:
        st.warning(
            ...
            "para activar la comparacion completa de modelos."
        )
...
def render_executive_view(
    ...
    with col_a:
        ...
        st.plotly_chart(fig, width="stretch")
    with col_b:
        risk_counts = health["Risk_Level"].astype(str).map(RISK_LABELS).value_counts().reindex(["Bajo", "Medio", "Alto"])
        fig = px.pie(
    ...
    render_model_metrics(risk_metrics, stock_metrics)
    st.info("Control de calidad: se excluye Overall_Risk_Score del modelo clinico para evitar data leakage.")


def render_inventory_view(stock: pd.DataFrame, stock_models: dict[str, object], stock_features: list[str]) -> None:
    ...
    alert_filter = c3.multiselect("Alertas", ["Critico", "Preventivo", "Normal"], default=["Critico", "Preventivo", "Normal"])
    ...
    if require_plotly():
        ...
        fig.add_trace(go.Scatter(x=item_df["Fecha"], y=item_df[projected_col], name=f"Proyeccion {horizon}", line=dict(color="#c1121f")))
        fig.add_trace(
            go.Scatter(
                x=item_df["Fecha"],
    ...
    st.dataframe(
        table[["ID_Insumo", "Stock_Actual", "Consumo_7d", "Lead_Time", "Cobertura_Dias", "Stock_Proyectado_7d", "Stock_Proyectado_14d", "Alerta"]],
        width="stretch",
        hide_index=True,
    )
    ...
    with st.expander("Simulador de compras y cobertura", expanded=True):
        ...
        ocupacion = s3.slider("Ocupacion del albergue", 0.0, 1.0, float(latest.get("Ocupacion_Albergue", 0.7)), 0.01)
        consumo14 = s4.number_input("Promedio 14 dias", min_value=0.0, value=float(latest["Consumo_14d"]), step=1.0)
        model_name = st.selectbox("Modelo", list(stock_models.keys()))
        stock_pred = stock_models[model_name].predict(
            pd.DataFrame([{"Consumo_Diario": consumo, "Lead_Time": lead, "Ocupacion_Albergue": ocupacion, "Consumo_7d": consumo, "Consumo_14d": consumo14}])[stock_features]
        )[0]
...
def render_clinical_view(
    ...
    cancer_filter = c2.multiselect("Tipo de cancer", sorted(health["Cancer_Type"].unique()), default=sorted(health["Cancer_Type"].unique())[:3])
    ...
    filtered = health[
        health["Risk_Level"].astype(str).isin(selected_raw_levels)
        & health["Cancer_Type"].isin(cancer_filter)
    ]
    if family_only:
    ...
    if require_plotly():
        ...
        with left:
            counts = filtered["Risk_Level"].astype(str).map(RISK_LABELS).value_counts().reindex(level_filter).fillna(0)
            fig = px.bar(x=counts.index, y=counts.values, color=counts.index, color_discrete_map=RISK_COLORS, labels={"x": "Prioridad", "y": "Pacientes"})
    ...
    st.dataframe(
        follow_up[["Patient_ID", "Cancer_Type", "Age", "BMI", "Family_History", "Risk_Lifestyle_Score", "Diet_Risk_Index", "Risk_Level"]],
        width="stretch",
        hide_index=True,
    )
    ...
    with st.expander("Matriz de confusion del modelo (validacion)"):
        model_name = st.selectbox("Modelo para matriz", list(risk_models.keys()), key="cm_model")
    ...
    with st.expander("Simulador de prioridad de paciente", expanded=True):
        ...
        model_name = st.selectbox("Modelo de clasificacion", list(risk_models.keys()))
        ...
        with r2:
            if require_plotly():
                fig = px.bar(proba_df, x="Prioridad", y="Probabilidad", color="Prioridad", color_discrete_map=RISK_COLORS)
...
def render_statistics_view(health: pd.DataFrame, stock: pd.DataFrame) -> None:
    ...
    st.caption("Exploracion interactiva alineada con el EDA del informe: distribuciones, correlaciones, outliers y resumen.")

    dataset = st.radio("Dataset", ["Clinico", "Inventario"], horizontal=True)
    stat_view = st.selectbox(
        "Tipo de analisis",
        ["Distribuciones", "Correlaciones", "Dispersion", "Outliers (Z-score)", "Resumen descriptivo", "Analisis por categoria"],
    )
    ...
    if dataset == "Clinico":
        if stat_view == "Distribuciones":
            variable = st.selectbox("Variable", ["Age", "BMI", "Risk_Lifestyle_Score", "Diet_Risk_Index", "Smoking", "Obesity"])
            fig = px.histogram(
                health,
                x=variable,
            ...
            cols = ["Age", "BMI", "Smoking", "Alcohol_Use", "Obesity", "Family_History", "Air_Pollution", "Risk_Lifestyle_Score", "Diet_Risk_Index"]
            ...
            fig = px.box(health, y=variable, color=health["Risk_Level"].astype(str).map(RISK_LABELS), color_discrete_map=RISK_COLORS)
            ...
            if view == "Historial familiar vs riesgo":
                grouped = health.groupby(["Family_History", health["Risk_Level"].astype(str).map(RISK_LABELS)]).size().reset_index(name="Pacientes")
                grouped.columns = ["Historial familiar", "Prioridad", "Pacientes"]
                fig = px.bar(grouped, x="Historial familiar", y="Pacientes", color="Prioridad", barmode="group", color_discrete_map=RISK_COLORS)
            else:
        ...
        if stat_view == "Distribuciones":
            ...
            fig = px.histogram(stock[stock["ID_Insumo"] == item], x=variable, nbins=35, color_discrete_sequence=["#1d3557"], marginal="box")
            fig.update_layout(height=480, margin=dict(l=10, r=10, t=20, b=10))
            st.plotly_chart(fig, width="stretch")
        elif stat_view == "Correlaciones":
            cols = ["Consumo_Diario", "Stock_Actual", "Lead_Time", "Consumo_7d", "Consumo_14d", "Cobertura_Dias", "Ocupacion_Albergue"]
            fig = px.imshow(stock[cols].corr(), text_auto=".2f", color_continuous_scale="RdBu_r", aspect="auto")
            fig.update_layout(height=620, margin=dict(l=10, r=10, t=20, b=10))
            st.plotly_chart(fig, width="stretch")
        elif stat_view == "Dispersion":
            fig = px.scatter(stock, x="Consumo_Diario", y="Stock_Actual", color="Alerta", color_discrete_map=ALERT_COLORS, opacity=0.5)
            fig.update_layout(height=480, margin=dict(l=10, r=10, t=20, b=10))
            st.plotly_chart(fig, width="stretch")
        elif stat_view == "Outliers (Z-score)":
...
def render_mlops_view(risk_metrics: pd.DataFrame, stock_metrics: pd.DataFrame) -> None:
...
[Streamlit Dashboard] --> KPIs, simuladores, menu estadistico, alertas
    ...
    architecture = pd.DataFrame(
        [
            ["Capa de datos", "Ingestion y versionado de CSV", "data/raw, data/merged, data/processed"],
...
def render_impact_ethics_view(health: pd.DataFrame, stock: pd.DataFrame) -> None:
    ...
    impact = pd.DataFrame(
        [
            ["ODS 3 - Salud", "Deteccion temprana de pacientes prioritarios", f"{high_risk_rate:.1%} del dataset en alto riesgo identificable"],
            ["ODS 10 - Equidad", "Perfil 360 con variables de condado", "Reduce sesgo por falta de contexto social"],
            ["Logistica", "Alertas preventivas de inventario", f"{critical_rate:.1%} de insumos en alerta critica hoy"],
            ["Eficiencia", "Automatizacion de analisis manual", "Libera tiempo del equipo para acompanamiento directo"],
            ["Seguridad alimentaria", "Control de insumos perecederos", "Proyecciones 7/14 dias para evitar desperdicio"],
        ],
    ...
    ethics = pd.DataFrame(
        [
            ...
            ["Falso negativo en alto riesgo", "Alto", "Priorizar F1 alto riesgo; revision humana obligatoria."],
            ["Data leakage (Overall_Risk_Score)", "Alto", "Variable excluida del modelo; auditoria continua."],
            ...
            ["Desbalance de clases", "Medio", "SMOTE en entrenamiento + metricas macro y por clase."],
        ],
...
def render_ecosystem_view() -> None:
    ...
    flow = pd.DataFrame(
        [
            ["01_descarga_datos.ipynb", "Ingestion desde Kaggle (salud + inventario)", "data/raw/"],
            ["02_merge_datasets.ipynb", "Fusion de datasets y enriquecimiento", "data/merged/"],
            ["Preprocesamiento.ipynb", "Limpieza, encoding, SMOTE, train/test", "data/processed/train.csv"],
            ["EDA.ipynb / Tarea_4.ipynb", "Analisis, modelado e interpretabilidad", "Metricas y features"],
            ["streamlit_app.py", "Capa de negocio: KPIs, alertas, simuladores", "Interfaz para directivos"],
        ],
    ...
    integration = pd.DataFrame(
        [
            ...
            ["Motor ML (este proyecto)", "Modelos entrenados y metricas de evaluacion", "Predicciones y alertas"],
...
risk_models, risk_metrics_df, risk_features, label_encoder, confusion_df = train_risk_models(health_df)
stock_models, stock_metrics_df, stock_features = train_stock_models(stock_df)
...
if section == "Resumen ejecutivo":
    render_executive_view(health_df, stock_df, risk_metrics_df, stock_metrics_df)
elif section == "Inventario predictivo":
    render_inventory_view(stock_df, stock_models, stock_features)
elif section == "Priorizacion clinica":
    render_clinical_view(health_df, risk_models, risk_features, label_encoder, confusion_df)
elif section == "Menu estadistico":
    render_statistics_view(health_df, stock_df)
elif section == "MLOps":
    render_mlops_view(risk_metrics_df, stock_metrics_df)
